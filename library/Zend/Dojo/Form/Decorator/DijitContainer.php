...
abstract class Zend_Dojo_Form_Decorator_DijitContainer extends Zend_Form_Decorator_Abstract
{
    ...
    public function getAttribs()
    {
        if (null === $this->_attribs) {
            ...
            if (array_key_exists('data-dojo-props', $attribs)) {
                //unset($attribs['data-dojo-props']);
            }
    ...
    {
        if (null === $this->_dijitProps) {
            $attribs = $this->getElement()->getAttribs();
            if (array_key_exists('data-dojo-props', $attribs)) {
                $this->_dijitProps = $attribs['data-dojo-props'];
            } else {
                $this->_dijitProps = array();
            }

            $options = $this->getOptions();
            if (array_key_exists('data-dojo-props', $options)) {
                $this->_dijitProps = array_merge($this->_dijitProps, $options['data-dojo-props']);
                $this->removeOption('data-dojo-props');
            }
        }

        // Ensure we have a title param
        if (!array_key_exists('title', $this->_dijitProps)) {
            $this->_dijitProps['title'] = $this->getTitle();
        }
    ...
    public function render($content)
    {
        ...
        if (array_key_exists('legend', $attribs)) {
            if (!array_key_exists('title', $dijitProps) || empty($dijitProps['title'])) {
                $dijitProps['title'] = $attribs['legend'];
            }
        ...
        return $view->$helper($id, $content, $dijitProps, $attribs);
    }
}
